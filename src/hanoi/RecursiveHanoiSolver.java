...
public class RecursiveHanoiSolver {
	...
	private void solverHelper(int n, int fromPeg, int toPeg, int auxPeg)
		...
		//Read pages 256-257 in the book to understand this recursive method
		//so my code is similar to the code in the book
		if(n<0)
			throw new IllegalHanoiMoveException("Illegal move, invalid # of rings");
		
		if(n > 0){
			solverHelper(n-1,fromPeg, auxPeg, toPeg);
			newBoard.doMove(new HanoiMove(fromPeg,toPeg));
	...
	public HanoiSolution solve(int n) throws IllegalHanoiMoveException {
		...
		newBoard = new ArrayBasedHanoiBoard(n);
